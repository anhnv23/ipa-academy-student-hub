...
export function AuthLogin({ onLogin }: { onLogin: (role: AppRole, name: string) => void }) {
  ...
  return <main className="login-page"><section className="login-card"><img src="/ipa-logo.jpg" alt="IPA English Academy"/><div className="login-copy"><span className="eyebrow">IPA ACADEMY • STUDENT HUB</span><h1>Đăng nhập hệ thống</h1><p>Dùng tài khoản do IPA cấp. Giao diện được mở tự động theo quyền Admin, Giáo viên hoặc Học viên.</p></div><form className="login-form" onSubmit={submit}><label><span>Email</span><div><Mail/><input type="email" required value={email} onChange={e=>setEmail(e.target.value)} placeholder="ten@ipaenglish.com"/></div></label><label><span>Mật khẩu</span><div><LockKeyhole/><input type="password" required value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••"/></div></label>{error&&<p style={{color:"#b42318"}}>{error}</p>}<button className="primary" disabled={busy}>{busy?"Đang đăng nhập…":"Đăng nhập"}</button>{!isSupabaseConfigured&&<small>Điền biến môi trường Supabase để kích hoạt đăng nhập.</small>}</form></section></main>;
}
